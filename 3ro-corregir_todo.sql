...
select id_cliente, nombre, apellido from clientes left join ventas on codigo left join articulos on codigo;

select producto from articulos group by;
...
select sum(cantidad) suma_total from ventas order by date desc;

-- 11) Informar cuántas unidades se vendieron de cada artículo.
select * from articulos cuantas_unidades left join ventas on cantidad group by cantidad;

-- 12) Informar quiénes compraron el primer día de ventas. *********** group by fecha orden decendente, puede ser 
select min(fecha) primer_dia_de_ventas from facturas;
select * from ventas where fecha = (min(fecha) from facturas);

-- 13) Informar la cantidad de facturas de cada cliente.
select * from facturas group by id_cliente;

-- 14) Agregar los campos precio_costo y precio_venta del tipo double a la tabla artículos.
-- alter table articulos add double precio_costo, precio_venta
...
select c.nombre, c.apellido from clientes c join vendedores v on v.nombre, v.apellido where email like "%gmail.com"; -- ***PREGUNTAR***

****************no con join, de otra manera**************
