...
-- select id_cliente, nombre, apellido from clientes where articulos 
...
select sum(cantidad) suma_total from ventas order by fecha desc;
...
select * from ventas where =(select min(fecha) from ventas);
...
select nombre, apellido from clientes where email = "%gmail.com"
union all
select nombre, apellido from vendedores where email= "%gmail.com"; -- ***PREGUNTAR***

-- 18) Listar el o los artículos que tengan la mayor cantidad de stock.
select max(stock_maximo) from articulos;
